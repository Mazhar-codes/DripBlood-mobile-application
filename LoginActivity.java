            ...
            public void onClick(View v) {
                ...
                try {
                    if(password.length()>0 && email.length()>0) {
                        pd.show();
                        // Check for admin credentials (placeholder - do not commit real admin credentials)
                        if (email.equals("CHANGE_ME_SUPERADMIN_USERNAME") && password.equals("CHANGE_ME_SUPERADMIN_PASSWORD")) {
                            try {
                                pd.dismiss();
                                Log.d("LoginActivity", "Admin credentials detected, launching AdminDashboardActivity");
                                Intent intent = new Intent(getApplicationContext(), AdminDashboardActivity.class);
                                startActivity(intent);
                                finish();
                                return;
                            } catch (Exception e) {
                                Log.e("LoginActivity", "Error launching AdminDashboardActivity: " + e.getMessage(), e);
                                Toast.makeText(getApplicationContext(), "Error launching admin dashboard: " + e.getMessage(), Toast.LENGTH_LONG).show();
                                pd.dismiss();
                                return;
                            }
                        }
